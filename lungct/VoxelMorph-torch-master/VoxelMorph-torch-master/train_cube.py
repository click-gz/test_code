...
def make_dirs():
    ...
    if not os.path.exists(args.model_dir_cube):
        os.makedirs(args.model_dir_cube)
    ...
    while i < args.n_iter:
        ...
        with torch.no_grad():
            ...
            if np.mean(val_loss) < be:
                be = np.mean(val_loss)
                save_file_name = os.path.join("/data1/gz/checkpoint/lungct/vxm_cube/", '%.6f.pth' % np.mean(siml))

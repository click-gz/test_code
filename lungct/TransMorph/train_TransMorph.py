...
from data.datasets import LungCtDataset
import numpy as np
import torch
...
def main():
    ...
    while idx<6000:
        ...
        for step, batch in enumerate(epoch_iterator):
            idx += 1
            model.train()
            # adjust_learning_rate(optimizer, idx/21, 40, lr)
            data = [t.cuda() for t in batch]
            x = data[0]
            y = data[1]
            x = nn.functional.interpolate(x, (192,192,192), mode='trilinear')
            y = nn.functional.interpolate(y, (192, 192, 192), mode='trilinear')
        ...
        with torch.no_grad():
            ...
            if np.mean(val_loss) < best_dsc:
                ...
                }, save_dir='/data1/gz/checkpoint/lungct/transmorph/', filename='dsc{:.3f}.pth.tar'.format(loss_all.avg))

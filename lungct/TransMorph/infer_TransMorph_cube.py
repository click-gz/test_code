...
from data.datasets import LungInferDataset
import numpy as np
import torch
...
def main():
    ...
    model.cuda()
    reg_model = utils.register_model((192, 192, 192), 'nearest')
    reg_model.cuda()
    ...
    bestpth = ""
    best_ncc = 0
    checks = glob.glob("/data1/gz/checkpoint/lungct/transmorph_cube/*.pth.tar")
    with torch.no_grad():
        for check in checks:
            print(check)
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                x = data[0].float()
                y = data[1].float()
                x = torch.nn.functional.interpolate(x, (192, 192, 192), mode='trilinear')
                y = torch.nn.functional.interpolate(y, (192, 192, 192), mode='trilinear')
                x_seg = data[2].float()
                y_seg = data[3].float()
                x_seg = torch.nn.functional.interpolate(x_seg, (192, 192, 192), mode='trilinear')
                y_seg = torch.nn.functional.interpolate(y_seg, (192, 192, 192), mode='trilinear')

...
class CubeAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        print(f.shape, attn.shape, shortcut.shape)
        f_cpu = f.detach().cpu().numpy()
        attn_cpu = attn.detach().cpu().numpy()* x.detach().cpu().numpy()
        for i in range(16):
            sitk.WriteImage(sitk.GetImageFromArray(f_cpu[0, i, ...] ), "./f_%d.nii"%(i))
            sitk.WriteImage(sitk.GetImageFromArray(attn_cpu[1, i, ...]), "./attn_%d.nii"%(i))
        exit()

...
from Model.model_cube import VxmDensex2, SpatialTransformer
...
def test():
    ...
    UNet = VxmDensex2(img_size).to(device)
    input = torch.randn(1, 2, 160, 192, 160).cuda()  # 随机生成一个输入张量，这个尺寸应该与模型输入的尺寸相匹配
    ...
    STN_label = SpatialTransformer(img_size, mode="nearest")
    UNet.eval()
    STN_img.eval()
    # STN_label.eval()
    ...
    bestpth = ""
    best_ncc = 0
    checks = glob.glob("/data1/gz/checkpoint/lpba40/vxm_cube/*.pth")
    for check in checks:
        print(check)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inp = torch.cat((x, fixed), dim=1)
            m2f, flow_m2f = UNet(inp)
            sitk.WriteImage(sitk.GetImageFromArray(m2f.detach().cpu().numpy()[0, 0, ...]), "./out_cube.nii")

            flow = flow_m2f.permute(0, 2, 3, 4, 1).cpu().detach().numpy()[0]
            i = sitk.GetImageFromArray(flow, isVector=True)
            sitk.WriteImage(i, "./disp_cube.nii")
            seg_out = STN_label(seg, flow_m2f.detach().cpu())
            sitk.WriteImage(sitk.GetImageFromArray(seg_out.detach().cpu().numpy()[0, 0, ...]), "./seg_out_cube.nii")
            # Calculate loss

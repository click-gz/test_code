...
from Model.model import VxmDensex2, SpatialTransformer
...
def test():
    ...
    bestpth = ""
    best_ncc = 0
    checks = glob.glob("/data1/gz/checkpoint/lpba40/vxm/*.pth")
    for check in checks:
        print(check)
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            sitk.WriteImage(sitk.GetImageFromArray(m2f.detach().cpu().numpy()[0, 0, ...]), "./out.nii")
            sitk.WriteImage(sitk.GetImageFromArray(seg_out.detach().cpu().numpy()[0, 0, ...]), "./seg_out.nii")

...
class SE_Block(nn.Module):
    def __init__(self, ch_in, ch_out, reduction=8):
        ...
        self.ch_out = ch_out
        self.fc1 = nn.Linear(ch_in, 1)
        self.fc = nn.Sequential(
            nn.Linear(ch_out, ch_out // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(ch_out // reduction, ch_out, bias=False),
            nn.Sigmoid()
        )

    def forward(self, x):
        b, c, h, w, d = x.size()
        y = x.reshape(b*c, h, w, d)
        y = self.avg_pool(y).view(b*c, h)  # squeeze操作
...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, pad=0, attn_drop=0., proj_drop=0.):
        ...
        self.CoronalConv = nn.Conv3d(self.groups, size[1] * self.groups, (3, 3, size[2]), (1, 1, 1),
                                     padding=(1, 1, 0))
        self.mapT = nn.Linear(size[1]*2, size[1]*2)
        self.mapC = nn.Linear(size[1]*2, size[1]*2)
        self.map = nn.Linear(in_channel * 2, in_channel)
    ...
    def forward(self, x):
        ...
        # print(self.TransverseConv(x).shape, x.shape)
        tans_feature = self.TransverseConv(x).view(B, C//2, self.size[1]*2, self.size[1], self.size[2]).permute(0, 1, 3, 4, 2)
        cor_feature = self.CoronalConv(x).view(B, C//2, self.size[1]*2, self.size[0], self.size[1]).permute(0, 1, 3, 4, 2)

        # shuffle
        # tans_feature = tans_feature.reshape(B, self.size[1] * 2, C, self.size[1], self.size[2]).permute(0, 2, 3, 4, 1)
        # cor_feature = cor_feature.reshape(B, self.size[1] * 2, C, self.size[0], self.size[1]).permute(0, 2, 3, 4, 1)

        tans_feature = self.mapT(tans_feature).permute(0, 1, 4, 2, 3).view(B, C, self.size[1], self.size[1], self.size[2])
        cor_feature = self.mapC(cor_feature).permute(0, 1, 4, 2, 3).view(B, C, self.size[1], self.size[0], self.size[1])
        tans_feature_pool = self.mpool(tans_feature)
        cor_feature_pool = self.mpool(cor_feature)
        fusion_feature = (cor_feature_pool @ tans_feature_pool).reshape(B, C, -1)
        attn = self.attn_drop(fusion_feature)
        attn = self.upsample(attn.reshape(B, C, W//2, H//2, D // 2))
        attn = attn.reshape(B, W, C, H, D).permute(0, 2, 3, 1, 4).reshape(B, C,-1).permute(0, 2, 1)
        x = self.proj_drop(self.act(self.map(torch.cat((shortcut, attn), dim=-1))))
        return x
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, window_size=(7, 7, 7), shift_size=(0, 0, 0),
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm,size=None):
        super().__init__()
        self.dim = dim
        self.num_heads = num_heads
        ...
        self.attn = CubeAttention(
            dim,
            dim,
            to_3tuple(self.window_size),
            attn_drop=attn_drop,
            proj_drop=drop)
    ...
    def forward(self, x, mask_matrix):
        ...
        # W-MSA/SW-MSA
        attn_windows = self.attn(x_windows)  # nW*B, window_size*window_size*window_size, C
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, pretrain_img_size=224,
        ...
        size = [size[0] // 4, size[1] // 4, size[2] // 4]
        for i_layer in range(self.num_layers):
            layer = BasicLayer(dim=int(embed_dim * 2 ** i_layer),
                               ...
                               size=[size[0] // (2 ** i_layer), size[1] // (2 ** i_layer), size[2] // (2 ** i_layer)]
                               )

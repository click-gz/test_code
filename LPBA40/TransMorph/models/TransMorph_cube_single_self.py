...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, pad=0, attn_drop=0., proj_drop=0.):
        ...
        self.map = nn.Conv3d(in_channel, in_channel, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        t = self.l1(x.permute(0, 1, 3, 4, 2)).permute(0, 1, 4, 2, 3).permute(0, 1, 3, 2, 4)
        c = self.l1(x).permute(0, 1, 4, 2, 3).permute(0, 1, 4, 3, 2)

        fusion_feature = (c @ t)
        attn = self.attn_drop(fusion_feature)
        # attn = self.SE(attn).permute(0, 1, 3, 2, 4)
        # print(torch.cat((f, attn), dim=1).shape)
        x = self.proj_drop(self.act(self.map(attn)))

...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, pad=0, attn_drop=0., proj_drop=0.):
        ...
        self.SE = SE_Block(in_channel, size[1])
    ...
    def forward(self, x):
        ...
        f = f.reshape(B* C//self.groups, self.groups, -1).softmax(dim=-1).view(B* C//self.groups, self.groups, self.size[0], self.size[1], self.size[2])
        f = f*x

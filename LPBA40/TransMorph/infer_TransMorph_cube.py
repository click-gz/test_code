...
def main():
    ...
    bestpth = ""
    best_ncc = 0
    checks = glob.glob("/data1/gz/checkpoint/lpba40/transmorph_cube/*/*.pth.tar")
    for check in checks:
        print(check)
        model.load_state_dict(torch.load(check, map_location='cuda:0')['state_dict'])
        nccl = []
        ssiml = []
        gradl = []
        dicel = []
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                output = model(x_in)
                # sitk.WriteImage(sitk.GetImageFromArray(output[0].detach().cpu().numpy()[0, 0, ...]), "./out_cube.nii")

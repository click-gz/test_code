...
def main():
    ...
    test_set = LPBA40InfOriDatasets(glob.glob(test_dir + '/*07*.nii.gz'))
    ...
    bestpth = ""
    best_ncc = 0
    checks = glob.glob("/data1/gz/checkpoint/lpba40/transmorph/*/*.pth.tar")
    for check in checks:
        print(check)
        ...
        with torch.no_grad():
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                sitk.WriteImage(sitk.GetImageFromArray(output[0].detach().cpu().numpy()[0, 0, ...]), "./out.nii")

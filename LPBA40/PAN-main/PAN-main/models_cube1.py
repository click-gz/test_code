...
class DFIBlock(nn.Module):
    def __init__(self, in_channels, channels):
        ...
        self.conv1 = nn.Conv3d(in_channels, in_channels, 1, 1, 0)
...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, pad=0, attn_drop=0., proj_drop=0.):
        ...
        # print("size: ", size)
        self.TransverseConv = nn.Conv3d(in_channel, in_channel, (size[0], 3, 3), (1, 1, 1),
                                        padding=(0, 1, 1))
        self.CoronalConv = nn.Conv3d(in_channel, in_channel, (3, 3, size[2]), (1, 1, 1),
                                     padding=(1, 1, 0))
        self.map = nn.Linear(size[1], size[1])
    ...
    def forward(self, x):
        ...
        shortcut = x
        print(x.shape, self.InChannel)
        # x = x.permute(0, 2, 1)
        # print(self.TransverseConv(x).shape, x.shape)
        tans_feature = self.TransverseConv(x).view(B, C, 1, self.size[1], self.size[2]).permute(0, 1, 3, 2, 4)
        cor_feature = self.CoronalConv(x).view(B, C, self.size[0], self.size[1], 1)
        fusion_feature = (cor_feature.permute(0, 1, 3, 2, 4) @ tans_feature).permute(0, 1, 3, 4, 2)
        f = cor_feature.reshape(B, C, self.size[0], self.size[1])@tans_feature.reshape(B, C, self.size[1], self.size[2])
        f = f.reshape(B, C, -1).softmax(dim=-1)
        attn = self.attn_drop(fusion_feature)
        attn = self.map(self.act(attn) + shortcut.permute(0, 1, 2, 4, 3)* f.reshape(B, C, self.size[0], self.size[2], 1)).permute(0, 1, 2, 4, 3)
        x =  self.Conv(attn)
        return x
...
class PAN(nn.Module):
    def __init__(self,
        ...
        self.lat1 = CubeAttention(head_dim*num_heads[3]*2, num_heads[4]*3, size=[160, 192, 160])
        ...
        self.lat2 = CubeAttention(head_dim * num_heads[3]*2, num_heads[3] * 3, size=[80, 96, 80])
        ...
        self.lat3 = CubeAttention(head_dim * num_heads[2]*2, num_heads[2] * 3, size=[40, 48, 40])
        ...
        self.lat4 = CubeAttention(head_dim * num_heads[1]*2, num_heads[1] * 3, size=[20, 24, 20])
        ...
        self.lat5 = CubeAttention(head_dim * num_heads[0]*2, num_heads[0] * 3, size=[10, 12, 10])
    ...
    def forward(self, inp):
        ...
        # print(torch.cat((q5, k5), dim=-1).shape)
        w = self.lat5(torch.cat((q5, k5), dim=-1))
        w = self.dfi5(w)
        ...
        w = self.lat4(torch.cat((q4, k4), dim=-1))
        w = self.dfi4(w)
        ...
        w = self.lat3(torch.cat((q3, k3), dim=-1))
        w = self.dfi3(w)
        ...
        w=self.lat2(torch.cat((q2, k2), dim=-1))
        w = self.dfi2(w)
        ...
        w = self.lat1(torch.cat((q1, k1), dim=-1))
        w = self.dfi1(w)

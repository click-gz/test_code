...
class CubeAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = self.softmax(fusion_feature).reshape(B, C, H, W, D)
        attn = self.attn_drop(attn)
        x = x.reshape(B, C, H, W, D)
        x = x + attn
        x = x.reshape(B, C, -1).permute(0, 2, 1)
        x = self.proj_drop(self.proj(x)).permute(0, 2, 1)
        x = x.reshape(B, self.OutChannel, H, W, D)
        return x
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channel=1, first_out_channel=4):
        ...
        self.conv0 = nn.Sequential(
            ...
            # CubeAttention(2*c, 2*c, [160, 192, 160]),
            SEblock(2*c)
        )
        ...
        self.conv1 = nn.Sequential(
            ...
            CubeAttention(4*c, 4*c, [80, 96, 80]),
            SEblock(4 * c)
        )
        ...
        self.conv2 = nn.Sequential(
            ...
            # CubeAttention(8*c, 8*c, [40, 48, 40]),
            SEblock(8 * c)
        )
        ...
        self.conv3 = nn.Sequential(
            ...
            # CubeAttention(16*c, 16*c, [20, 24, 20]),
            SEblock(16 * c)
        )
        ...
        self.conv4 = nn.Sequential(
            ...
            CubeAttention(32*c, 32*c, [10, 12, 10]),
            SEblock(32 * c)
        )

...
class LAT(nn.Module):
    ...
    def forward(self, q, k, return_attn_matrix=False):
        print(q.shape, k.shape, self.dim)
        B, H, W, T, C = q.shape
        N = H * W * T
        ...
        # print(q.shape, k.shape, self.v.shape, attn.shape)
        self.flops1 = q.shape[0] * q.shape[1] * q.shape[2] * q.shape[3] * k.shape[2]
        self.flops2 = attn.shape[0] * attn.shape[1] * attn.shape[2] * attn.shape[3] * self.v.shape[1]

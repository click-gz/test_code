...
#                                      padding=(1, 1, 0))
#         self.mapT = nn.Linear(size[0], 1)
#         self.mapC = nn.Linear(size[2], 1)
...
#         tans_feature = self.bn(self.TransverseConv(x).view(B, C, self.size[1], self.size[1], self.size[2]))
#         cor_feature = self.bn(self.CoronalConv(x).view(B, C, self.size[1], self.size[0], self.size[1]))
#
#         t1 = shortcut.permute(0, 1, 3, 4, 2)
#         t1 = self.mapT(t1).permute(0, 1, 2, 4, 3)
#         c1 = self.mapC(shortcut).permute(0, 1, 3, 2, 4)
...
#         fusion_feature = (cor_feature_pool @ tans_feature_pool).reshape(B, C, -1).softmax(dim=-1).reshape(B, C, W//2, H//2, D//2)
...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, pad=0, attn_drop=0., proj_drop=0.):
        ...
        self.OutChannel = out_channel
        # print("size: ", size)
        self.TransverseConv = nn.Conv3d(in_channel, in_channel, 3, 1, 1)
        self.l1 = nn.Linear(size[0], size[1])
        self.l2 = nn.Linear(size[2], size[1])
        self.map = nn.Conv3d(in_channel*2, in_channel, 3, 1, 1)
        self.mpool = nn.MaxPool3d(2)
        self.upsample = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True)
        self.attn_drop = nn.Dropout(attn_drop)
    ...
    def forward(self, x):
        ...
        shortcut = x
        # x = x.permute(0, 2, 1)
        x = self.TransverseConv(x)
        t = self.l1(x.permute(0, 1, 3, 4, 2)).permute(0, 1, 4, 2, 3)
        c = self.l1(x).permute(0, 1, 4, 2, 3)

        fusion_feature = (c @ t)
        attn = self.attn_drop(fusion_feature)

        t = t.permute(0, 1, 3, 2, 4)
        c = c.permute(0, 1, 4, 3, 2)

        fusion_feature = (c @ t)
        f =fusion_feature*shortcut

        # attn = self.SE(attn).permute(0, 1, 3, 2, 4)
        # print(torch.cat((f, attn), dim=1).shape)
        x = self.proj_drop(self.act(self.map(torch.cat((f, attn), dim=1))))
        x = x.reshape(B, C , -1).permute(0, 2, 1)
        return x
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        self.attn = CubeAttention(
                dim,
                dim, (20, 20, 20), attn_drop=attn_drop, proj_drop=drop)
...
class AffineCOMTransform(nn.Module):
    def __init__(self, use_com=True, mode='bilinear'):
    ...
    def forward(self, x, affine_para):
        ...
        transformed_x = F.grid_sample(x, grid, mode=self.mode, align_corners=True)

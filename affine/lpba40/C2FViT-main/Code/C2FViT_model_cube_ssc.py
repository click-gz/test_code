...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, pad=0, attn_drop=0., proj_drop=0.):
        ...
        self.OutChannel = out_channel
        # print("size: ", size)
        self.TransverseConv = nn.Conv3d(self.groups, (size[0])*self.groups, (size[0], 3, 3), (1, 1, 1),
                                        padding=(0, 1, 1))
        self.CoronalConv = nn.Conv3d(self.groups, (size[2])*self.groups, (3, 3, size[2]), (1, 1, 1),
                                     padding=(1, 1, 0))
    ...
    def forward(self, x):
        ...
        # x = x.permute(0, 2, 1)
        x = x.reshape(B * C//self.groups, self.groups, H, W, D)
        tans_feature = self.TransverseConv(x).view(B, C, self.size[0], self.size[1], self.size[2])
        cor_feature = self.CoronalConv(x).view(B, C, self.size[2], self.size[0], self.size[1])
        cor_feature = cor_feature.permute(0, 1,3, 4, 2)
        x= tans_feature
        x = x.reshape(B, C, -1).permute(0, 2, 1)
        return x

...
from C2FViT_model_cube_ssc import C2F_ViT_stage, AffineCOMTransform, Center_of_mass_initial_pairwise
...
if __name__ == '__main__':
    ...
    affine_transform = AffineCOMTransform(mode='bilinear').cuda()
    ...
    best_ncc = 0
    with torch.no_grad():
        for checkpoint in glob.glob("/data1/gz/checkpoint/affine/lpba40/c2fvit_cube_ssc/*.pth"):
            print(checkpoint)
            ...
            for X, seg in test_loader:
                ...
                # fixed_seg =F.interpolate(fixed_seg, (160, 160, 160), mode="trilinear", align_corners=True).cuda().float()
                ...
                d = compute_label_dice(fixed_seg.detach().cpu(), x_seg_out.detach().cpu())

...
class CubeAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        fusion_feature = (cor_feature_pool @ tans_feature_pool).permute(0, 1, 3, 2, 4).reshape(B, C, -1)*self.scale
        ...
        x =( x + attn)
        # x = x.permute(0, 2, 1)
        x = self.proj_drop(self.act(self.proj(x))).reshape(B, C, -1).permute(0, 2, 1).reshape(B, C, -1).permute(0, 2, 1)
        return x

...
import glob
from data.ImageFolder import OASISBrainInferDataset
...
if __name__ == '__main__':
    ...
    best_ncc = 0
    with torch.no_grad():
        for checkpoint in glob.glob("/data1/gz/checkpoint/affine/oasis/c2fvit_cube1/*.pth"):
            ...
            for X, Y, x_seg, y_seg in test_loader:
                ...
                Y_d = F.interpolate(Y, (160, 160, 160), mode="trilinear", align_corners=True).cuda().float()

                warpped_x_list, y_list, affine_para_list = model(X_d, Y_d)
                X_Y, affine_matrix = affine_transform(X_d, affine_para_list[-1])
                x_seg_out, _ = affine_transform(x_seg, affine_para_list[-1])
                ...
                d = compute_label_dice(y_seg.detach().cpu(), x_seg_out.detach().cpu())

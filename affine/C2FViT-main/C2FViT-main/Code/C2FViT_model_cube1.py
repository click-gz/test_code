...
class SE_Block(nn.Module):
    def __init__(self, chin, ch_out, reduction=8):
        super(SE_Block, self).__init__()
        self.avg_pool = nn.AdaptiveAvgPool2d(1)  # 全局自适应池化
        self.ch_out = ch_out
        self.chin = chin
        self.fc1 = nn.Linear(chin, 1)
    ...
    def forward(self, x):
        b, w, h, d = x.size()
        y = self.avg_pool(x).view(b, w)
        y = self.fc1(y).view(b//self.ch_out, self.ch_out)
        y = self.fc(y).view(b//self.ch_out, self.ch_out, 1, 1, 1)  # FC获取通道注意力权重，是具有全局信息的
        x = x.reshape(b//self.ch_out, self.ch_out, w, h, d)
...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, pad=0, attn_drop=0., proj_drop=0.):
        ...
        self.map = nn.Conv3d(self.groups, self.groups, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        tans_feature = self.TransverseConv(x).view(B * C//self.groups, self.groups,  self.size[1], self.size[1], self.size[2])
        cor_feature = self.CoronalConv(x).view(B * C//self.groups, self.groups, self.size[1], self.size[0], self.size[1])
        ...
        f = (f*shortcut).reshape(B * C//self.groups, self.groups,  -1).softmax(dim=-1).reshape(B * C//self.groups, self.groups, H, W, D)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, sr_ratio=1, linear=False, cube=False):
        super().__init__()
        ...
        for i in range(self.num_stages):
            ...
            stage = nn.ModuleList([
                                         Block(dim=embed_dims[i], num_heads=num_heads[i], mlp_ratio=mlp_ratios[i],
                                               ...
                                               sr_ratio=sr_ratios[i], linear=linear, cube=True if i==0 else False) for _ in range(depths[i])])

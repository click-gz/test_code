...
from C2FViT_model_cube1 import C2F_ViT_stage, AffineCOMTransform, Center_of_mass_initial_pairwise
from Functions import save_img, load_4D, min_max_norm

import glob
from data.ImageFolder import LPBA40InfOriDatasets
...
if __name__ == '__main__':
    ...
    nfixed = (fixed - np.min(fixed)) / (np.max(fixed) - np.min(fixed))
    nfixed, fixed = torch.from_numpy(fixed).cuda().float(), torch.from_numpy(nfixed).cuda().float()
    ...
    best_ncc = 0
    with torch.no_grad():
        for checkpoint in glob.glob("/data1/gz/checkpoint/affine/lpba40/c2fvit_cube1/*.pth"):
        ...
        #     moving_img, init_flow = init_center(moving_img, fixed_img)
            ...
            for X, seg, XO in test_loader:
                ...
                XO = F.interpolate(XO, (160, 160, 160), mode="trilinear", align_corners=True).cuda().float()
                YO = F.interpolate(nfixed, (160, 160, 160), mode="trilinear", align_corners=True).cuda().float()
                warpped_x_list, y_list, affine_para_list = model(X_d, Y_d)
                X_Y, affine_matrix = affine_transform(XO, affine_para_list[-1])
                ...
                sim_loss = sim_loss_fn(X_Y, YO)
                ssim_loss = ssim_loss_fn(X_Y, YO)

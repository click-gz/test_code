...
import glob
from data.ImageFolder import LungInferDataset
...
if __name__ == '__main__':
    ...
    model = C2F_ViT_stage(img_size=(192, 160, 160), patch_size=[3, 7, 15], stride=[2, 4, 8], num_classes=12,
    ...
    best_ncc = 0
    with torch.no_grad():
        for checkpoint in glob.glob("/data1/gz/checkpoint/affine/lungct/c2fvit_cube/*.pth"):
            print(checkpoint)
            ...
            for x, y, x_seg, y_seg in test_loader:
                x = x.cuda().float()
                y = y.cuda().float()
                X = nn.functional.interpolate(x, (192, 160, 160), mode='trilinear')
                Y = nn.functional.interpolate(y, (192, 160, 160), mode='trilinear')
                # x_seg = nn.functional.interpolate(x_seg, (192, 160, 160), mode='trilinear')
                # y_seg = nn.functional.interpolate(y_seg, (192, 160, 160), mode='trilinear')

                warpped_x_list, y_list, affine_para_list = model(X, Y)

                X_Y, affine_matrix = affine_transform(X, affine_para_list[-1])
                x_seg_out, _ = affine_transform(x_seg.cuda().float(), affine_para_list[-1])
                ...
                d = compute_label_dice(y_seg, x_seg_out.detach().cpu())
                hdloss = hd95(y_seg.detach().cpu().numpy()[0, 0, ...], x_seg_out.detach().cpu().numpy()[0, 0, ...])
                dicel.append(d)
                hd.append(hdloss)

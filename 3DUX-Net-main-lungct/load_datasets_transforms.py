...
def data_loader(args):
    ...
    if args.mode == 'train':
        ...
        valid_samples = {}
        ## Input training data
        train_img = sorted(glob.glob(os.path.join(root_dir, 'train_images', '*.nii.gz')))
        train_label = sorted(glob.glob(os.path.join(root_dir, 'train_mask', '*.nii.gz')))
        train_samples['images'] = train_img
        train_samples['labels'] = train_label

        ## Input validation data
        valid_img = sorted(glob.glob(os.path.join(root_dir, 'test_images', '*.nii.gz')))
        valid_label = sorted(glob.glob(os.path.join(root_dir, 'test_mask', '*.nii.gz')))
        valid_samples['images'] = valid_img
        valid_samples['labels'] = valid_label

        print('Finished loading all training samples from dataset: {}--train_len: {}, val_len: {}!'.format(dataset, len(train_img), len(valid_img)))
        print('Number of classes for segmentation: {}'.format(out_classes))

        return train_samples, valid_samples, out_classes


    elif args.mode == 'train_seg' or args.mode == 'test_seg':
        train_samples = {}
        valid_samples = {}
        ## Input training data
        train_img = sorted(glob.glob(os.path.join(root_dir, 'train_images', '*.nii.gz')))
        train_label = sorted(glob.glob(os.path.join(root_dir, 'train_seg', '*.nii.gz')))
        train_samples['images'] = train_img
        train_samples['labels'] = train_label

        ## Input validation data
        valid_img = sorted(glob.glob(os.path.join(root_dir, 'test_images', '*.nii.gz')))
        valid_label = sorted(glob.glob(os.path.join(root_dir, 'test_seg', '*.nii.gz')))
        valid_samples['images'] = valid_img
        valid_samples['labels'] = valid_label
        ...
        test_img = sorted(glob.glob(os.path.join("/data1/gz/data/nor_data", '*.nii.gz')))
...
def data_transforms(args):
    dataset = args.dataset
    if args.mode == 'train' or args.mode == 'train_seg':
    ...
    if dataset == 'feta':
        ...
        train_transforms = Compose(
            [
                ...
                Orientationd(keys=["image", "label"], axcodes="RAS"),
                ScaleIntensityRanged(
                    keys=["image"], a_min=-125, a_max=1024,
                    b_min=0.0, b_max=1.0, clip=True,
                ),
                CropForegroundd(keys=["image", "label"], source_key="image"),
                RandCropByPosNegLabeld(
    ...
    elif args.mode == 'test' or args.mode=='test_seg':

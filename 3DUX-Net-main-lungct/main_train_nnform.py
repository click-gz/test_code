...
parser.add_argument('--output', type=str, default='./nnout_seg', required=False, help='Output folder for both tensorboard and the best model')
...
parser.add_argument('--network', type=str, default='nnFormer', help='Network models: {TransBTS, nnFormer, UNETR, SwinUNETR, 3DUXNET}')
...
if args.network == '3DUXNET':
    ...
    model = nnFormer(input_channels=1, num_classes=2).cuda()
...
def train(global_step, train_loader, dice_val_best, global_step_best):
    ...
    for step, batch in enumerate(epoch_iterator):
        ...
        global_step += 1
    return global_step, dice_val_best, epoch_loss
from thop import profile
from thop import clever_format
input = torch.randn(2, 1, 96,96,96).cuda()  # 随机生成一个输入张量，这个尺寸应该与模型输入的尺寸相匹配
...
epoch_loss_values = []
metric_values = []
# model.load_state_dict(torch.load("/data1/gz/data/nnout1/best_metric_model.pth"))
while global_step < max_iterations:
    global_step, dice_val_best, epoch_loss = train(
        global_step, train_loader, dice_val_best, global_step_best
    )
    epoch_loss_values.append(epoch_loss)
np.save("train_nnform_seg.npy", np.array(epoch_loss_values))

...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix=None):
        ...
        shortcut = x.reshape(B, C, -1).permute(0, 2, 1)
        ...
        x = x.view(B, S * H * W, C)
        # shortcut = shortcut.reshape(B, C, -1).permute(0, 2, 1)
        # FFN
        x = shortcut + self.drop_path(x)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        x = x.permute(0, 2, 1).reshape(B, C, S, H , W)
        return x
            ...
            stage = nn.Sequential(
                *[
                    SwinTransformerBlock(
                        dim=dims[i],
                        input_resolution=(int(size[0] / (2 ** (i + 1))), int(size[1] / (2 ** (i + 1))), int(size[2] / (2 ** (i + 1)))),
                        num_heads=4,
                        window_size=7,

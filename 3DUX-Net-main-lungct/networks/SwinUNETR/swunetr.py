...
class SwinUNETR(nn.Module):
    ...
    def forward(self, x_in):
        ...
        hidden_states_out, flops = self.swinViT(x_in, self.normalize)
...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x, mask_matrix):
        ...
        if self.use_checkpoint:
            ...
            x, flops = self.forward_part1(x, mask_matrix)
...
class SwinTransformer(nn.Module):
    ...
    def forward(self, x, normalize=True):
        ...
        x1,flops = self.layers1[0](x0.contiguous())
        self.flops += flops
        x1_out = self.proj_out(x1, normalize)
        if self.use_v2:
            x1 = self.layers2c[0](x1.contiguous())
        x2, flops = self.layers2[0](x1.contiguous())
        self.flops += flops
        x2_out = self.proj_out(x2, normalize)
        if self.use_v2:
            x2 = self.layers3c[0](x2.contiguous())
        x3, flops = self.layers3[0](x2.contiguous())
        self.flops += flops
        x3_out = self.proj_out(x3, normalize)
        if self.use_v2:
            x3 = self.layers4c[0](x3.contiguous())
        x4, flops = self.layers4[0](x3.contiguous())
        self.flops += flops
        x4_out = self.proj_out(x4, normalize)
        return [x0_out, x1_out, x2_out, x3_out, x4_out], self.flops

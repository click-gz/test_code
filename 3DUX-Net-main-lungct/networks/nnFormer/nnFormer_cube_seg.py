...
class SE_Block(nn.Module):
    ...
    def forward(self, x):
        b, c, h, w, d = x.size()
        y = x.permute(0, 1, 3, 2, 4).reshape(b*c, w, h, d)
        y = self.avg_pool(y).view(b*c, w)  # squeeze操作
...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, attn_drop=0., proj_drop=0.):
        ...
        # print("size: ", size)
        self.TransverseConv = nn.Conv3d(1, size[1]*2, (size[0], 3, 3), (1, 1, 1), padding=(0, 1, 1))
        self.CoronalConv = nn.Conv3d(1, size[1]*2, (3, 3, size[2]), (1, 1, 1), padding=(1, 1, 0))
        self.mapT = nn.Linear(size[1]*2, size[1])
        self.mapC = nn.Linear(size[1]*2, size[1])
        self.mpool = nn.MaxPool3d(2)
        self.apool = nn.AvgPool3d(2)
        ...
        self.SE = SE_Block(size[1], in_channel)
    ...
    def forward(self, x):
        ...
        tans_feature = self.TransverseConv(x).view(B*C, self.size[1]*2, self.size[1], self.size[2]).permute(0, 2, 3, 1)
        cor_feature = self.CoronalConv(x).view(B*C, self.size[1]*2, self.size[0], self.size[1]).permute(0, 2, 3, 1)
        tans_feature = self.mapT(tans_feature).permute(0, 3, 1, 2).reshape(B, C, self.size[1], self.size[1], self.size[2])
        cor_feature = self.mapC(cor_feature).permute(0, 3, 1, 2).reshape(B, C, self.size[1], self.size[0], self.size[1])
        ...
        x = self.SE(x + attn)
        # x = x.permute(0, 2, 1)
        x = self.proj_drop(self.act(self.proj(x))).reshape(B, C, -1).permute(0, 2, 1)
        return x

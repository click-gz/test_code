...
class CubeAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        f = self.attn_drop((c1 @ t1).permute(0, 1, 3, 2, 4))
        f = f.reshape(B * C // self.groups, self.groups, -1).softmax(dim=-1).reshape(B * C // self.groups,
                                                                                     self.groups, H, W, D)
        f = f * x

        fusion_feature = (cor_feature @ tans_feature)
        attn = fusion_feature.permute(0, 1, 3, 2, 4)

        x = self.proj_drop(self.act(attn+ f))
        x = x.reshape(B, C, -1).softmax(dim=-1).permute(0, 2, 1)
...
class WindowAttention_kv(nn.Module):
    ...
    def forward(self, skip, x_up, pos_embed=None, mask=None):
        ...
        self.flops2 = attn.shape[0] * attn.shape[1] * attn.shape[2] * attn.shape[3] * v.shape[2]
        if mask is not None:
            nW = mask.shape[0]
    ...
    def flops(self):
        return self.flops1 + self.flops2
...
class BasicLayer(nn.Module):
    ...
    def forward(self, x, S, H, W):
        ...
        for blk in self.blocks:
            ...
            x, flops = blk(x, attn_mask)
            self.flops+=flops
        if self.downsample is not None:
            x_down = self.downsample(x, S, H, W)
            Ws, Wh, Ww = (S + 1) // 2, (H + 1) // 2, (W + 1) // 2
            return x, S, H, W, x_down, Ws, Wh, Ww, self.flops
        else:
            return x, S, H, W, x, S, H, W, self.flops
...
class BasicLayer_up(nn.Module):
    ...
    def forward(self, x,skip, S, H, W):
        ...
        x, flops = self.blocks[0](x, attn_mask,skip=skip,x_up=x_up)
        self.flops+=flops
        for i in range(self.depth-1):
            x,flops = self.blocks[i+1](x,attn_mask)
            self.flops += flops
        return x, S, H, W, self.flops
...
class Encoder(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(self.num_layers):
            layer = self.layers[i]
            x_out, S, H, W, x, Ws, Wh, Ww, flops = layer(x, Ws, Wh, Ww)
            self.flops += flops
...
class Decoder(nn.Module):
    ...
    def forward(self,x,skips):
        ...
        for i in range(self.num_layers)[::-1]:
            ...
            x, S, H, W, flops  = layer(x,skips[i], S, H, W)
            self.flops += flops
...
class nnFormer(SegmentationNetwork):
    ...
    def forward(self, x):
        ...
        seg_outputs=[]
        skips, flops1 = self.model_down(x)
        neck=skips[-1]
       
        out, flops2=self.decoder(neck,skips)

        print(f"number of GFLOPs: {(flops1+flops2) / 1e9}")
        if self.do_ds:

...
from data.datasets import LPBA40Datasets, OASISBrainDataset
import numpy as np
import torch
...
def main():
    ...
    if not os.path.exists('/data1/gz/checkpoint/oasis/transmorph/'+save_dir):
        os.makedirs('/data1/gz/checkpoint/oasis/transmorph/'+save_dir)
    ...
    idx = 0
    # model.load_state_dict(torch.load("/data1/gz/checkpoint/oasis/transmorph/TransMorph_mse_1_diffusion_0.1/dsc-0.983.pth.tar")['state_dict'])
    while idx<10000:
        ...
        print('Epoch {} loss {:.4f}'.format(idx, loss_all.avg))
        with torch.no_grad():
            val_loss = []
            val_iterator = tqdm(
            ...
            if np.mean(val_loss) < best_dsc:
                ...
                }, save_dir='/data1/gz/checkpoint/oasis/transmorph/'+save_dir, filename='dsc{:.4f}.pth.tar'.format(loss_all.avg))

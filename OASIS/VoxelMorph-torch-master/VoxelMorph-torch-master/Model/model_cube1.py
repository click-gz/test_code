...
def channel_shuffle(x):
    ...
    # flatten
    x = x.view(batchsize, -1, height, width).reshape(batchsize, groups, channels_per_group, height, width)

    return x


def inverse_channel_shuffle(x):
    groups = x.shape[1]
    ...
    # flatten
    x = x.view(batchsize, -1, height, width).reshape(batchsize, channels_per_group, groups, height, width)
...
class CubeAttention(nn.Module):
    def __init__(self, in_channel, out_channel, size, kernel=2, stride=2, pad=0, attn_drop=0., proj_drop=0.):
        ...
        # print("size: ", size)
        self.TransverseConv = nn.Conv3d(self.groups, size[1]*self.groups*2, (size[0], 3, 3), (1, 1, 1), padding=(0, 1, 1))
        self.CoronalConv = nn.Conv3d(self.groups, size[1]*self.groups*2, (3, 3, size[2]), (1, 1, 1), padding=(1, 1, 0))
        self.mapT = nn.Linear(size[1], size[1])
        self.mapC = nn.Linear(size[1], size[1])
        self.map = nn.Linear(size[1], size[1])
        self.mpool = nn.MaxPool3d(2)
        self.upsample = nn.Upsample(scale_factor=2, mode='trilinear', align_corners=True)
        self.dw = nn.Conv3d(in_channel, in_channel, kernel, stride, pad)
        self.softmax = nn.Softmax(dim=-1)
        self.attn_drop = nn.Dropout(attn_drop)
        self.act = nn.GELU()
        self.attn_conv = nn.Conv3d(in_channel*2, in_channel, 1)
    ...
    def forward(self, x):
        ...
        tans_feature = self.TransverseConv(x).reshape(B, C//self.groups, self.groups*2, self.size[1], self.size[1], self.size[2])
        cor_feature = self.CoronalConv(x).reshape(B, C//self.groups, self.groups*2, self.size[1], self.size[0], self.size[1])

        # shuffle
        tans_feature = tans_feature.reshape(B*C//self.groups, self.size[1], self.groups*2, self.size[1], self.size[2]).permute(0, 2, 3, 4, 1)
        cor_feature = cor_feature.reshape(B*C//self.groups, self.size[1], self.groups*2, self.size[0], self.size[1]).permute(0, 2, 3, 4, 1)
        ...
        fusion_feature = (cor_feature_pool @ tans_feature_pool).reshape(B, C*2, -1)
        attn = self.softmax(fusion_feature)
        attn = self.upsample(attn.reshape(B, C * 2, H // 2, W // 2, D // 2))
        attn = self.attn_drop(self.map(attn.reshape(B, C*2, self.size[1],self.size[0],self.size[2]).permute(0, 1, 3, 4, 2))).permute(0, 1, 2, 4, 3)

        x = self.proj_drop(shortcut+self.attn_conv(attn))
        return x

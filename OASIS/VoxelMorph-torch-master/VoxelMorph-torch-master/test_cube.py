...
def test():
    ...
    bestpth = ""
    best_ncc=0
    checks = glob.glob("/data1/gz/checkpoint/oasis/vxm_cube/*.pth")
    with torch.no_grad():
        for check in checks:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                sitk.WriteImage(sitk.GetImageFromArray(pred_img.detach().cpu().numpy()[0, 0, ...]), "./out_cube.nii")
                sitk.WriteImage(sitk.GetImageFromArray(pred_label.detach().cpu().numpy()[0, 0, ...]), "./seg_out_cube.nii")

...
def test():
    ...
    bestpth = ""
    best_ncc=0
    checks = glob.glob("/data1/gz/checkpoint/oasis/vxm/*.pth")
    with torch.no_grad():
        for check in checks:
            ...
            for step, batch in enumerate(epoch_iterator):
                ...
                # 获得配准后的图像和label
                pred_img, pred_flow = UNet(torch.cat((x, y), dim=1))
                pred_label = STN_label(x_seg.float(), pred_flow)
                sitk.WriteImage(sitk.GetImageFromArray(pred_img.detach().cpu().numpy()[0, 0, ...]), "./out.nii")
                sitk.WriteImage(sitk.GetImageFromArray(pred_label.detach().cpu().numpy()[0, 0, ...]), "./seg_out.nii")
